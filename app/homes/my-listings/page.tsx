    ...
    const fetchData = async () => {
      try {
        ...
        if (homesResponse.ok) {
          ...
          // Filter to only show current user's homes
          const myHomes = homesData.homes.filter((home: any) => home.owner.id === profileData.user.id)
          ...
          <div className="space-y-4">
            {userHomes.map((home) => (
              <Link
                ...
                className="block bg-[#1A202C]/80 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-[#E8D5B7]/20 hover:border-[#E8D5B7]/40 transition-all transform hover:-translate-y-1"
              >
                ...
                <div className="flex items-center justify-between pt-4 border-t border-[#E8D5B7]/20">
                  ...
                  <div className="flex items-center gap-6 text-sm text-[#E8D5B7]/70">
                    <span>{home.bedrooms} {getTranslation(language, 'bedrooms').toLowerCase()}</span>
                    <span>{home.bathrooms} {getTranslation(language, 'bathrooms').toLowerCase()}</span>
